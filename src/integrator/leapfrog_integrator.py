...
class LeapfrogIntegrator(Integrator):
    ...
    def __init__(self, kinetic_energy_instance, potential_instance, step_size=1, number_of_integration_steps=10):
        ...
        super().__init__(kinetic_energy_instance=kinetic_energy_instance, potential_instance=potential_instance,
                         step_size=step_size, number_of_integration_steps=number_of_integration_steps)
    ...
    def flow(self, support_variable, momentum, charges=None):
        ...
        numpy_array
            The flow.
        """
        support_variable_flow = np.empty((len(support_variable), self._number_of_integration_steps))
        momentum_flow = np.empty((len(momentum), self._number_of_integration_steps))
        support_variable_flow[:, 0] = support_variable
        momentum_flow[:, 0] = momentum
        for i in range(self._number_of_integration_steps):
            intermediate_momentum = (momentum_flow[:, i] - 0.5 * self._step_size *
                                     self._potential_instance.gradient(support_variable_flow[:, i], charges=charges))
            support_variable_flow[:, i + 1] = (support_variable_flow[:, i] + self._step_size *
                                               self._kinetic_energy_instance.gradient(intermediate_momentum))
            momentum_flow[:, i + 1] = (intermediate_momentum - 0.5 * self._step_size *
