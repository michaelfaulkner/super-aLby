...
def main(config_file_string):
    ...
    potential, number_of_equilibration_iterations, temperatures, samplers, config_file_mediator = (None, None, None,
                                                                                                   None, None)
    for possible_mediator in possible_mediators:
        try:
            ...
            temperatures = other_methods.get_temperatures(
                parsing.get_value(config, possible_mediator, "minimum_temperature"),
                parsing.get_value(config, possible_mediator, "maximum_temperature"),
                parsing.get_value(config, possible_mediator, "number_of_temperature_values"))
    ...
    sample_directory = config.get(strings.to_camel_case(samplers[0]), "output_directory")
    try:
                ...
                for sampler in samplers:
                    if sampler == "standard_mean_position_sampler":
                        ...
                        magnetic_norm_density_mean_and_error = get_sample_mean_and_error(
                            sample_getter.get_magnetic_norm_density(sample_directory, temperature, temperature_index,
                                                                    number_of_particles,
                        ...
                        magnetic_norm_susceptibility_mean_and_error = get_sample_mean_and_error(
                            sample_getter.get_magnetic_norm_susceptibility(sample_directory, temperature,
    ...
    if potential != "ising_potential":
        ...
        if "coulomb" in potential or "lennard_jones" in potential:
            sample = sample_getter.get_particle_separations(sample_directory, temperatures[0], 0, 2,
                                                            number_of_equilibration_iterations).flatten()
        else:

            sample = sample_getter.get_positions(sample_directory, temperatures[0], 0, 2,
                                                 number_of_equilibration_iterations).flatten()
